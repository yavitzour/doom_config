...
(defun org-offer-all-agenda-tags ()
  (setq-local org-complete-tags-always-offer-all-agenda-tags t))
(add-hook 'org-agenda-mode-hook 'org-offer-all-agenda-tags)
...
;; Renderer
(defconst +agenda-projects-not-task-faces '(("STRT" . '(:inherit org-todo :weight normal))
                                            ("TODO" . '(:inherit org-todo :weight normal))))

(defconst +agenda-projects-task-faces '(("STRT" . '(:inherit org-todo :weight bold))
                                        ("TODO" . '(:inherit org-todo :weight bold))))

(defun +agenda-format-entry (prefix entry)
  (let ((props (list 'my-custom-agenda t
    ...
    (when (or +agenda-show-private
      ...
      (if (not todo-type)
        ...
        (if (or (eq todo-type 'done)
          ...
          (cond
           ;; NOTE(nox): Planned
           ((and (not (string= todo "STRT")) scheduled-future)
